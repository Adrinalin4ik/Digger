...
{  
	...
	RenderWindow window(VideoMode(resX, resY), "Digger",Style::Default);
	//Текустуры
	Texture SkillBarTexture;
	SkillBarTexture.loadFromFile("SkillBar.png");
    Sprite SkillBar(SkillBarTexture);
	SkillBar.setPosition(resX/4,resY-100);
   ...
   // music.play();
	...
	Texture menu_texture1,menu_texture2,menu_texture3,about_texture,victory_texture, texture_backgroundMenu;
	...
	Sprite backgroundMenu(texture_backgroundMenu),menu1(menu_texture1),menu2(menu_texture2),menu3(menu_texture3),about(about_texture), victory_sprite(victory_texture);
	bool Menu=true;
	int MenuNum=0;
	int x,y,x1,y1,x2,y2;
	menu1.setPosition(x=resX/2-100,y=resY/2-60);
	menu2.setPosition(x1=resX/2-100,y1=resY/2);
    menu3.setPosition(x2=resX/2-100,y2=resY/2+60);
	...
	{	   
       ...
       if (IntRect(x2,y2,300,50).contains(Mouse::getPosition(window))) {menu3.setColor(Color::Yellow); MenuNum=3;}

	   if (Mouse::isButtonPressed(Mouse::Left))
	     {
			 if (MenuNum==1) {Menu=false;Player.collection_count = 0;}
			 if (MenuNum==2) {about.setPosition(x,y);window.draw(about); window.display(); while(!Keyboard::isKeyPressed(Keyboard::Escape)) ;}
